...
{ //O que está fazendo: Define o início de uma função chamada retornaDadosUsuario. Esta função provavelmente tem o objetivo de retornar os dados de um usuário com base no ID armazenado na sessão.
    ...
    try {
        $sql = "SELECT * FROM usuarios WHERE email = ?"; //O que está fazendo: Cria uma variável $sql que contém a consulta SQL. A consulta é um SELECT que busca todos os dados da tabela usuarios onde o campo id é igual a um valor que será passado dinamicamente (representado pelo ?).
        ...
        $stmt->execute([$_SESSION['email']]); //O que está fazendo: A consulta preparada na linha anterior é executada com um parâmetro. O valor de $_SESSION['id'] (que provavelmente é o ID do usuário logado armazenado na sessão) é passado para a consulta, substituindo o ? na cláusula WHERE id = ? da consulta SQL.
...
function alterarDadosUsuario($nome, $email)
{
    ...
    try {
        ...
        if ($stmt->execute([$nome, $email, $_SESSION['email']])) {
            echo "<p class='text-success'> Dados alterados com sucesso!</p>";
            $_SESSION['email'] = $email; // atualiza o email na sessão, senão as futuras alterações não funcionarão
        } else {
    ...
    try {
        if ($novaSenha == $novaSenhaConfirm) {
            ...
            if (password_verify($senhaAntiga, $usuario['senha'])) {
                ...
                if ($stmt->execute([$novaSenha, $_SESSION['email']])) {
                    require("sair.php");
                } else {
